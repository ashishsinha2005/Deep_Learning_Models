...
# Use the correct device for your tensors
# new_boxes = torch.cat((xy.to(device), confidences.unsqueeze(1).to(device), classes.unsqueeze(1).to(device)), 1)

cap = cv2.VideoCapture(r"D:\HOPE\Projects\Helmet_detection\Helmet_detection_with_number_plate\22.mp4")  # For videos

model = YOLO("runs/detect/train/weights/best.pt") # after training update the location of best.pt
